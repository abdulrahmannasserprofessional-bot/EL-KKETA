        ...
        function showQuestion() {
            ...
            } else if (q.options && q.options.length === 2 && (q.options[0].includes('صح') || q.options[0].includes('خطأ') || q.options[0].includes('True') || q.options[0].includes('False'))) {
                badgeText = '⚖️ صح أم خطأ';
            }
            ...
            if (q.type === 'essay') {
                ...
                textarea.style.cssText = 'width: 100%; min-height: 150px; padding: 15px; border-radius: 15px; background: rgba(0,0,0,0.05); border: 1px solid rgba(0,0,0,0.1); color: var(--text-main); font-family: inherit; font-size: 16px; margin-bottom: 15px; resize: vertical; outline: none; transition: border 0.3s;';
                ...
                submitBtn.style.cssText = 'width: 100%; padding: 16px; background: linear-gradient(135deg, #3B82F6, #2563EB); color: var(--text-main); border: none; border-radius: 15px; font-family: inherit; font-size: 16px; font-weight: bold; cursor: pointer; transition: 0.2s;';
                ...
                let leftItems = q.pairs.map(p => p.left);
                leftItems.sort(() => Math.random() - 0.5); // Shuffle options

                const table = document.createElement('div');
                table.style.cssText = 'display: flex; flex-direction: column; gap: 15px; margin-bottom: 20px;';

                rightItems.forEach((rightText, idx) => {
                    const row = document.createElement('div');
                    row.style.cssText = 'display: flex; align-items: center; justify-content: space-between; background: rgba(0,0,0,0.02); padding: 12px; border-radius: 10px; border: 1px solid rgba(0,0,0,0.1); gap: 10px; flex-wrap: wrap;';
                    
                    const label = document.createElement('div');
                    label.textContent = rightText;
                    label.style.cssText = 'flex: 1; min-width: 120px; font-weight: bold; color: var(--text-main); font-size: 15px;';
                    
                    const select = document.createElement('select');
                    select.className = 'matching-select';
                    select.dataset.correct = q.pairs[idx].left;
                    select.style.cssText = 'flex: 1; min-width: 150px; padding: 10px; border-radius: 8px; border: 1px solid #ccc; background: white; font-family: inherit; font-size: 14px;';
                ...
                const submitBtn = document.createElement('button');
                submitBtn.textContent = 'تأكيد التوصيل';
                ...
                submitBtn.onclick = () => checkMatchingAnswer(q);
        ...
        function checkMatchingAnswer(q) {
            ...
            if (!allAnswered) {
                showToast("يرجى توصيل كل العناصر قبل الإرسال", "warning");
                return;
            }
            
            // Disable selects and color them
            selects.forEach(sel => {
                sel.disabled = true;
                if (sel.value === sel.dataset.correct) {
            ...
            try {
                ...
                qRef.child('correctAnswer').set('سؤال توصيل (Matching)');
            ...
            let feedHTML = `
                <div class="feedback-status">
                    ${isCorrect ? '✅ <span style="color:#22c55e;">إجابة صحيحة! أحسنت 🌟</span>' : '❌ <span style="color:#ef4444;">إجابة خاطئة في بعض أو كل الاختيارات</span>'}
                </div>
            `;
            
            if (!isCorrect) {
                feedHTML += `<div class="correct-answer-box" style="margin-top: 10px;">💡 التوصيل الصحيح:<br>`;
                q.pairs.forEach(p => {
                    feedHTML += `<div style="margin-top:5px; font-size:14px;">- <b>${p.right}</b> ⬅️ <b>${p.left}</b></div>`;
                });
                feedHTML += `</div>`;
            }
