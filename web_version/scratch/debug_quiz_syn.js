        ...
        function showQuestion() {
            ...
            // Set question text
            document.getElementById('questionText').innerHTML = q.text || 'بدون نص';
            ...
            if (q.type === 'essay') {
                ...
                document.querySelector('.question-type-badge').textContent = badgeText;
                
                const textarea = document.createElement('textarea');
                textarea.id = 'essayAnswerText';
                textarea.style.cssText = 'width: 100%; height: 150px; padding: 15px; border-radius: 12px; border: 2px solid var(--border-color); font-size: 16px; font-family: inherit; resize: vertical; margin-bottom: 20px;';
                textarea.placeholder = 'اكتب إجابتك هنا...';
                
                const submitBtn = document.createElement('button');
                submitBtn.textContent = 'إرسال الإجابة';
                submitBtn.className = 'btn-nav btn-next';
                submitBtn.style.width = '100%';
                submitBtn.onclick = () => submitEssayAnswer(q);
                
                container.appendChild(textarea);
                container.appendChild(submitBtn);
                
            } else if (q.type === 'matching') {
                badgeText = '🔗 سؤال توصيل';
                document.querySelector('.question-type-badge').textContent = badgeText;
                    ...
                    btn.onclick = () => {
                        if (selectedRightIdx !== null) {
                            ...
                            if (linkedRIdx !== undefined) {
                                ...
                                showToast("قم باختيار عنصر من القائمة الأولى أولاً");
                            }
                        }
                    };
                ...
                (q.options || []).forEach((opt, idx) => {
                    ...
                    if (window.userAnswers && window.userAnswers[currentIndex] === idx) {
                        inp.checked = true;
                    ...
                    inp.addEventListener('change', () => {
                        ...
                        lbl.classList.add('selected');
                        // Do not immediately check answer unless we want to, wait for next button?
                        // Actually in this quiz, clicking an option immediately checks it usually.
        ...
        function submitEssayAnswer(q) {
            ...
            feedHTML += `
                <button class="btn-nav btn-next" id="nextBtn" style="margin-top:20px;">${currentIndex < questions.length - 1 ? 'السؤال التالي ⬅️' : '🎉 إنهاء الامتحان'}</button>
            `;
        ...
        function checkMatchingAnswerGrid(q) {
            ...
            try {
                ...
                qRef.child('explanation').set(q.explanation || '?? ???? ?????');
                    ...
                    qRef.child(`choices/إجابة_خاطئة_في_التوصيل`).set(firebase.database.ServerValue.increment(1));
                }
            } catch(err) {
            ...
            let feedHTML = `
                <div class="feedback-status">
                    ${isCorrect ? '✅ <span style="color:#22c55e;">إجابة صحيحة! أحسنت 🌟</span>' : '❌ <span style="color:#ef4444;">إجابة خاطئة. لا بأس حاول في القادم!</span>'}
                </div>
            `;
            
            if (!isCorrect) {
                feedHTML += `<div class="correct-answer-box">`;
                q.pairs.forEach(p => {
                    feedHTML += `
                    <div class="matching-correct-item">
                        <div class="mc-left">${p.left}</div>
                        <div class="mc-icon">🔗</div>
                        <div class="mc-right">${p.right}</div>
                    </div>`;
                });
                feedHTML += `</div>`;
            }
            ...
            feedHTML += `<button class="btn-nav btn-next" id="nextBtn" style="margin-top:20px;">${currentIndex < questions.length - 1 ? '?????? ?????? ??' : '?? ????? ????????'}</button>`;
            feedCard.innerHTML = feedHTML;
        ...
        function checkAnswer(selectedIdx) {
            ...
            feedHTML += `<button class="btn-nav btn-next" id="nextBtn" style="margin-top:20px;">${currentIndex < questions.length - 1 ? 'السؤال التالي ⬅️' : '🎉 إنهاء الامتحان'}</button>`;
            feedCard.innerHTML = feedHTML;
        ...
        function finishQuiz() {
            ...
            if (user && user.studentCode) {
                ...
                const examNameForLog = window.currentExamId || urlParams.get('examId') || urlParams.get('subject') || 'امتحان';
                logRef.set({
                    studentCode: user.studentCode,
                    studentName: user.fullName || 'طالب',
                    action: `أنهى امتحان (${examNameForLog}) وحصل على ${pct}%`,
    ...
    q.type = q.type || 'mcq';
    q.correct = q.correctAnswerIndex !== undefined ? q.correctAnswerIndex : (q.correct !== undefined ? q.correct : 0);
    return q;
});
                                    startTimer();
                                    showQuestion();
    ...
    return q;
            ...
            // Try to parse from sessionStorage or URL
            let examJsonStr = urlParams.get('exam');
            if (urlParams.get('fromStorage') === 'true') {
                examJsonStr = sessionStorage.getItem('currentExamData');
            }
            if (examJsonStr) {
                try {
                    // It might be URI encoded if from URL, or plain if from sessionStorage
                    // But JSON.parse can handle plain JSON strings.
                    // If it throws URI error, we decode. If it throws syntax error, maybe it was not encoded.
                    let decodedStr = examJsonStr;
                    try {
                        decodedStr = decodeURIComponent(examJsonStr);
                    } catch(e) {}
    ...
    return q;
        ...
        if (stUser && localDeviceId) {
            database.ref('ActiveSessions/' + stUser.studentCode).on('value', snap => {
                if(snap.exists() && snap.val() !== localDeviceId) {
                    alert("⚠️ تم تسجيل الدخول من جهاز آخر! سيتم تسجيل خروجك وتأمين حسابك.");
