        ...
        function downloadOfflineExam(lessonTitle, lessonIdOrCode) {
            try {
                ...
                const htmlParts = [
                    ...
                    '        :root { --primary: #4F46E5; --primary-dark: #3730A3; --bg: #F8FAFC; --surface: #FFFFFF; --text-main: #1E293B; --text-sub: #64748B; --border: #E2E8F0; --success: #10B981; --danger: #EF4444; --warning: #F59E0B; }',
                    '        body { background: var(--bg); color: var(--text-main); min-height: 100vh; padding-bottom: 50px; }',
                    '        .header { background: linear-gradient(135deg, #1E3A8A 0%, #3B82F6 100%); color: white; padding: 25px 20px; text-align: center; border-bottom-left-radius: 25px; border-bottom-right-radius: 25px; box-shadow: 0 10px 25px rgba(59, 130, 246, 0.2); position: sticky; top: 0; z-index: 100; }',
                    ...
                    '        .q-card { background: white; border-radius: 20px; padding: 25px; margin-bottom: 20px; border: 1px solid var(--border); box-shadow: 0 4px 15px rgba(0,0,0,0.03); transition: all 0.2s; }',
                    ...
                    '        .opt-btn { background: #F8FAFC; border: 2px solid var(--border); border-radius: 14px; padding: 14px 18px; text-align: right; font-size: 15px; font-weight: 700; cursor: pointer; transition: all 0.2s; color: var(--text-main); display: flex; align-items: center; justify-content: space-between; }',
                    ...
                    '        .explanation-box { background: #EFF6FF; border: 1px solid #BFDBFE; border-radius: 12px; padding: 14px; margin-top: 15px; font-size: 14px; color: #1E40AF; display: none; }',
                    '        .submit-btn { width: 100%; padding: 18px; background: linear-gradient(135deg, #10B981, #059669); color: white; border: none; border-radius: 16px; font-size: 18px; font-weight: 900; cursor: pointer; box-shadow: 0 8px 25px rgba(16,185,129,0.3); transition: all 0.2s; margin-top: 10px; }',
                    '        .submit-btn:hover { transform: translateY(-2px); box-shadow: 0 12px 30px rgba(16,185,129,0.4); }',
                    '        .score-modal { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.6); display: none; align-items: center; justify-content: center; padding: 20px; z-index: 1000; }',
                    '        .score-modal.active { display: flex; }',
                    '        .score-card { background: white; border-radius: 24px; padding: 35px 25px; text-align: center; max-width: 400px; width: 100%; box-shadow: 0 20px 40px rgba(0,0,0,0.2); }',
                    '        .score-val { font-size: 42px; font-weight: 900; color: var(--primary); margin: 15px 0 5px; }',
                    '        .btn-retry { background: var(--primary); color: white; border: none; padding: 12px 24px; border-radius: 14px; font-weight: 800; font-family: "Cairo", sans-serif; width: 100%; font-size: 15px; margin-top: 15px; cursor: pointer; }',
                    '    </style>',
                    '</head>',
                    '<body>',
                    '    <div class="header">',
                    '        <h1>' + safeTitle + '</h1>',
                    '        <p>منصة الخطة التعليمية — نسخة بدون إنترنت (أوفلاين)</p>',
                    '    </div>',
                    '    <div class="container">',
                    ...
                    '                var bodyHtml = "";',
                    '                if (qType === "essay") {',
                    '                    bodyHtml = "<textarea id=\'ans_" + idx + "\' placeholder=\'اكتب إجابتك هنا بالتفصيل...\' rows=\'3\' style=\'width:100%; padding:12px; border:2px solid #E2E8F0; border-radius:12px; font-size:15px; outline:none;\'></textarea><div class=\'explanation-box\' id=\'exp_" + idx + "\'><strong>الإجابة النموذجية:</strong><br>" + (q.modelAnswer || q.explanation || "لا يوجد تفسير متاح") + "</div>";',
                    '                } else {',
                    '                    var options = q.options || [];',
                    '                    var optsHtml = options.map(function(opt, oIdx) {',
                    '                        return "<button class=\'opt-btn\' id=\'opt_" + idx + "_" + oIdx + "\' onclick=\'selectOption(" + idx + ", " + oIdx + ")\'><span>" + opt + "</span><span class=\'opt-mark\' style=\'font-size:14px;\'>⚪</span></button>";',
                    '                    }).join("");',
                    '                    bodyHtml = "<div class=\'options-list\'>" + optsHtml + "</div><div class=\'explanation-box\' id=\'exp_" + idx + "\'>💡 <strong>التفسير والتوضيح:</strong><br>" + (q.explanation || "لا يوجد تفسير إضافي") + "</div>";',
                    '                }',
                    '                card.innerHTML = "<div class=\'q-num\'>السؤال " + (idx + 1) + " من " + questions.length + "</div><div class=\'q-text\'>" + (q.text || q.question || "") + "</div>" + bodyHtml;',
                    ...
                    '                var btn = document.getElementById("opt_" + qIdx + "_" + i);',
                    '                if (btn) {',
                    ...
                    '            var total = 0;',
                    '            questions.forEach(function(q, idx) {',
                    '                var exp = document.getElementById("exp_" + idx);',
                    ...
                    '                    var userSelected = answers[idx];',
                    '                    if (userSelected !== undefined) {',
                    '                        var userBtn = document.getElementById("opt_" + idx + "_" + userSelected);',
                    '                        if (userSelected == correctIdx) {',
                    '                            score += 1;',
                    '                            if (userBtn) userBtn.classList.add("correct");',
                    '                        } else {',
                    '                            if (userBtn) userBtn.classList.add("wrong");',
                    '                            var correctBtn = document.getElementById("opt_" + idx + "_" + correctIdx);',
                ...
                const blob = new Blob([htmlParts], { type: 'text/html;charset=utf-8' });
                const downloadUrl = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = downloadUrl;
                a.download = 'امتحان_' + safeTitle + '_أوفلاين.html';
                document.body.appendChild(a);
                a.click();
                document.body.removeChild(a);
                URL.revokeObjectURL(downloadUrl);

                showToast("✅ تم تنزيل الامتحان بنجاح كملف HTML أوفلاين!", "success");

            } catch (err) {
                console.error(err);
                showToast("⚠️ حدث خطأ أثناء تجهيز ملف الامتحان", "error");
            }
        }
