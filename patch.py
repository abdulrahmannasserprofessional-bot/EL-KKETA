...
match_new = '''            } else {
                ...
                let mistakes = JSON.parse(localStorage.getItem('mistakes_list') || '[]');'''
c = c.replace(match_old, match_new)
...
mcq_new = '''            } else {
                ...
                // Save to mistakes bank
                let mistakes = JSON.parse(localStorage.getItem('mistakes_list') || '[]');'''
c = c.replace(mcq_old, mcq_new)

# 4. showDetailedReport
report_func = '''
...
'''
c = c.replace('        // ─── Finish Quiz ───', report_func + '\n        // ─── Finish Quiz ───')

# 5. Overwrite finishQuiz call
finish_old = '''            const resultScreen = document.getElementById('resultScreen');
            resultScreen.style.display = 'block';
            
            if (autoQuestions === 0 && pendingEssays.length > 0) {'''
finish_new = '''            // Show detailed report instead of default screen
            ...
            if (autoQuestions === 0 && pendingEssays.length > 0) {'''
c = c.replace(finish_old, finish_new)
